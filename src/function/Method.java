...
public class Method {
    ...
    public static void downloadFile(int ID, String name) {
        try {
            ...
            int c = ch.showSaveDialog(Client.getFrames()[0]);
            if (c == JFileChooser.APPROVE_OPTION) {
                File f = ch.getSelectedFile();
                if (f.exists()) {
                    int click = JOptionPane.showConfirmDialog(Client.getFrames()[0], "This file name has already do you want to replace", "Save File", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
